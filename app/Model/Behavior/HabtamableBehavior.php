...
class HabtamableBehavior extends ModelBehavior {
	...
	private function changeBind(&$Model) {
		...
		$Model -> bindModel(array('hasOne' => array(
				$Model -> hasAndBelongsToMany[$this -> settings[$Model -> alias]['habtmModel']]['with'] => array(
					...
					'conditions' => array($Model -> hasAndBelongsToMany[$this -> settings[$Model -> alias]['habtmModel']]['with'] . '.' . $Model -> hasAndBelongsToMany[$this -> settings[$Model -> alias]['habtmModel']]['associationForeignKey'] . ' = ' . $Model -> alias . '.' . $Model -> primaryKey)
				),
				$this -> settings[$Model -> alias]['habtmModel'] => array(
					'foreignKey' => FALSE,
					'type' => $this -> settings[$Model -> alias]['joinType'],
					'conditions' => array($this -> settings[$Model -> alias]['habtmModel'] . '.' . $Model -> {$this -> settings[$Model -> alias]['habtmModel']} -> primaryKey . ' = ' . $Model -> hasAndBelongsToMany[$this -> settings[$Model -> alias]['habtmModel']]['with'] . '.' . $Model -> hasAndBelongsToMany[$this -> settings[$Model -> alias]['habtmModel']]['foreignKey'])
				)
			)));
	}
	...
	private function getExistingId(&$Model) {
		...
		$existingRecord = $Model -> {$this -> settings[$Model -> alias]['habtmModel']} -> find('first', array(
